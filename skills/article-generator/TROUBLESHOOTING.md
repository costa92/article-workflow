...
# 检查环境变量
echo $GEMINI_API_KEY
# Or check config: cat config/config.json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('gemini_api_key','NOT SET'))"
...
# 编辑 config/config.json，设置 gemini_api_key
```
...
# 或: 编辑 config/config.json，设置 gemini_api_key
```
...
1. 环境变量 GEMINI_API_KEY
   ↓ (如果不存在)
2. config/config.json 中的 gemini_api_key (推荐) ⭐
   ↓ (如果不存在)
3. ~/.claude/env.json 中的 gemini_api_key (向后兼容，仅旧版)
   ↓ (如果不存在)
4. 报错: "Missing GEMINI_API_KEY"
```

**推荐做法：** 设置环境变量 `GEMINI_API_KEY`，或在 `config/config.json` 中配置，模板为 `config/config.example.json`。
