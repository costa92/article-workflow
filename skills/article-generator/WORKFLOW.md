...
5. 🎨 图片生成（如果请求）
   ├─ 图片类型选择：工具/CLI 教程优先截图，概念文章用 AI 插图，评测文章用真实截图
...
6. 最终审查
   ├─ 验证所有链接正常（HTTP 200）
   ├─ 验证文章正文中的 URL 可达（grep 提取 + curl 批量检测）
   ├─ 验证文章中的 CLI 命令正确（非白名单命令跑 --help 确认）
   ├─ 确认所有代码示例完整
   ├─ 检查没有 AI 陈词滥调或营销废话
   ├─ 确保 YAML frontmatter 完整
   └─ 质量门控：「快速入门」「实战教程」「深度解析」是深度选项，均需跑 reviewer + SEO
       仅用户明确说"测试""草稿""先看看"才跳过
```
