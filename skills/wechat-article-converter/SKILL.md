...
>
> **Default Author**: When generating any draft JSON for WeChat (create_draft or convert --draft), set "author" from the ARTICLE_AUTHOR environment variable or config (default: "Anonymous") unless the user explicitly specifies a different author.
...
# 指定作者和封面
python3 ${SKILL_DIR}/scripts/upload_draft.py article.md --theme tech --author "Your Name" --cover cover.jpg
```
...
| `--author` | From config / Anonymous | 文章作者 |
...
| `ARTICLE_AUTHOR` | `default_author` | 文章默认作者 | 草稿上传 |
...
- **content-reviewer**: 文章质量审查
- **nanobanana-skill**: 图片生成
- *content-repurposer* (可选外部 skill): 多平台内容分发
- *content-analytics* (可选外部 skill): 文章数据分析
