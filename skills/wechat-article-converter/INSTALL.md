...
通过环境变量或插件 `config/config.json` 配置：
...
或在插件根目录的 `config/config.json` 中配置对应字段：`wechat_appid`、`wechat_secret`、`image_api_key`、`image_api_base`
...
# 下载对应平台的二进制文件到缓存目录（如 ~/.cache/claude/bin/ 或系统对应的缓存路径）
```
