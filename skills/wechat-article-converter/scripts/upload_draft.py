...
用法：
    ...
    python3 upload_draft.py article.md --theme coffee --author "Your Name" --cover cover.jpg
"""
...
def build_and_upload_draft(html_content, metadata, cover_media_id, author=None):
    ...
    if author is None:
        ...
        try:
            ...
            author = os.environ.get("ARTICLE_AUTHOR", "Anonymous")
    print(f"\n{'='*60}")
...
示例:
  python3 upload_draft.py article.md --theme coffee
  python3 upload_draft.py article.md --theme tech --author "Your Name"
  python3 upload_draft.py article.md --theme coffee --cover cover.jpg
    ...
    parser.add_argument("--author", default=None, help="文章作者 (默认: 从配置读取或 Anonymous)")
    ...
    # 检查环境变量（支持 config_loader）
    sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', 'shared'))
    try:
        from config_loader import get_config, export_to_env
        export_to_env("wechat_appid", "wechat_secret")
    except ImportError:
        pass
    ...
    if not appid or not secret:
        ...
        print("  或在 config/config.json 中配置 wechat_appid / wechat_secret")
        sys.exit(1)
